...
export default function TypeFilter({ types, selected, onChange }) {
  ...
  const label = useMemo(() => {
    if (selected.size === 0) return "Todos os tipos";
    if (selected.size === 1) return [...selected][0];
    return `${selected.size} tipos`;
  }, [selected]);
        ...
        {open && (
          ...
          >
            <div className="project-filter-actions">
              ...
              <button onClick={() => onChange(new Set(types))}>Selecionar todos</button>
            </div>
            {types.length === 0 && <div className="empty-state">Nenhum tipo carregado.</div>}
            {types.map((name) => (
